...
int main() {
    ...
    auto material_left   = make_shared<dielectric>(INDEX_DIAMOND);
    auto material_bubble = make_shared<dielectric>(1.00/INDEX_DIAMOND);
    ...
    for (int i = 0; i < 30;i++) {
        double t = random_double(0,2) * i;
        world.add(make_shared<sphere>(point3(t+1,t+1,t+1),   0.5, material_right));
    }
    camera dolly{IMG_WIDTH,IMG_HEIGHT,vec3{-3,2.5,2.2},vec3{0,0,-1},2};
    BVHNode root = BVHNode(world);
    // dolly.renderSDL(root);
